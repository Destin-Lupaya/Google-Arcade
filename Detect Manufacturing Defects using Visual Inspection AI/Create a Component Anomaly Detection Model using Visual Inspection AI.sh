...
# PCBs (printed circut boards) in general are assemblies of components, including through-hole and surface mounted. Components can be discrete parts placed within the assembly or physical features created within the assembly such as solder joints, glue, or welds. For subassemblies of electronic products, components can be screws, other hardware parts, or whole other subassemblies.

# In this lab you will use Visual Inspection AI to prepare acomponent anomaly detection dataset and then create and train a component anomaly detection model.

# Model training can take a a long time so this lab is paired with Deploy and Test a Visual Inspection AI Component Anomaly Detection Solution where you deploy a Visual Inspection solution artifact from this lab and then use that artifact to generate inferences about sample images.
...
# Review the process involved in evaluation of trained Assembly Inspection anomaly detection model.
...
# Itself is not anomalous or has not been damaged during placement.
...
# The Visual Inspection AI UI open on the Browse tabbed page, which displays a tiled view of images
...
# The zoomed in view of an image 
...
# The Using as template toggle highlighted on the UI

# Once the template image is defined, you need to specify an inspection area within the template image where all components to be inspected by the automated algorithm reside. The purpose of defining the inspection area via bounding box regions is to allow the Visual Inspection AI's algorithms to focus only on modelling the pixels that are relevant (Included) and ignore any pixels that are irrelevant (Excluded), such as background pixels.
...
# An image displaying one included and one excluded inspection area

# Click Save.

# Detect and crop components via alignment
# Once the inspection area is defined, you need to define individual components on the template image for the subsequent component modelling.
...
# Click Done.
# The Components tabbed page displaying the tiled images mentioned previously
...
# Click the Template image for close-up view.

# Click the Add bounding box tool from the tool bar.

# Draw a region for one Resistance component.

# One image displaying the popup with various options
...
# Note: In the example image below, you will see that there is also a Soldering Component. You do not need to create this component, but you should be aware that model training is per component, and typically you would have more than a single component type. For the purposes of this lab, will use only the Resistance component type.
# The Components page displaying the highlighted Detect Components button
...
# The Components tabbed page displaying a selection of cropped resistance images

# If you are satisfied with the preview results of detected and cropped component regions, you can move to the next steps.

# Click the Apply to all status icon on the right side of the user interface panel, and then click the Apply button to formally apply the detection and cropping processes to all images. This will trigger another long running task and the Apply to all task status icon on the right side of the user interface panel indicates that the process is running.
...
# You have now completed the steps required to prepare for final model training. The training process will also take some time, well over an hour for this demo dataset. For this reason the rest of this lab provides an overview of the remaining steps involved in evaluating and refining a model and creating the solution artifact that can then be used to analyse images. In the follow up lab to this one, Deploying and Testing a Visual Inspection AI Component Anomaly Detection Solution, you learn how to deploy and use this solution artifact to analyse images.
...
# Clicking Suggested to label under Resistance switches to component images used during training that Visual Inspection AI has identified as the most useful for manual labelling.
...
# The images surrounded by the Suggested to label boxes are the images that the Visual Inspection AI system believes are most likely to need attention and verification, that is human labelling. If you follow these Suggested to label images, and verify them as either NORMAL or ABNORMAL, the human labelling information you provide on these images allows the Visual Inspection AI system to train a better Anomaly Detection model for the labelled component in subsequent rounds of Anomaly Detection model training.

# Some of the images in the demo dataset are deliberately masked to trigger defect detection using a blue rectangular mask and are used in this demo to make it easy to identify abnormal components. Labelling normal and abnormal components manually following a round of training helps to improve the accuracy of the model.

# The images on the first page of Selected to label that contain a blue rectangle masking the resistor can be labelled as ABNORMAL by clicking the Label as abnormal option that appears in the pop-up above the images. These abnormal images are then highlighted in red.

# Any remaining unassigned images on the first page of the Selected to label can then be labelled as NORMAL by clicking the Label as normal option that appears in the pop-up above the images. These normal images are then highlighted in green.

# Abnormal component images highlighted in red and normal component images highlighted in green

# There are also two tabs, Unlabelled and Labelled, on the left panel that allow you to easily navigate through all the component images that you have already labelled under the Labelled tab, and the images you have not labelled yet under the Unlabelled tab.

# Once NORMAL or ABNORMAL labels have been selected you can trigger a new round of Anomaly Detection model training to use these manually labelled components to improve the model.
...
# Once the component's Anomaly Detection model has been trained, the trained model's performance can be checked from Model page. There are two ways to navigate to the model evaluation page.
...
# The Create Solution Artifact dialog box
...
# The Solution Artifact details on the Test and Use tabbed page
...
# The Create batch prediction dialog box
...
# Finish your quest
# This self-paced lab is part of the Detect Manufacturing Defects using Visual Inspection AI skill badge quest. A quest is a series of related labs that form a learning path. Completing this quest earns you a badge to recognize your achievement. You can make your badge or badges public and link to them in your online resume or social media account. Enroll in this quest and get immediate completion credit. Refer to the Google Cloud Skills Boost catalog for all available quests.
...
# Lab Last Tested: February 15,2023
