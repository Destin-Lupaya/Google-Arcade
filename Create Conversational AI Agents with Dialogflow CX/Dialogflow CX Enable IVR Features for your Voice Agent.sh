...
# Note: If prompted, click Enable API.
...
# After creating the agent, navigate to Agent Settings > General > Logging settings and click on Enable Cloud Logging option. It will generate logs for this agent.
...
# Click on select file and click Restore.
...
# Slide right to enable each of the following:
...
# Scroll down to the Advanced speech settings and choose Enable DTMF. For now, assume the confirmation number is usually a 5 digit number such as "12345".
...
# On the Fulfillment page, scroll down to Advanced speech settings and find Barge-in. Since this fulfillment provides the key information for the passenger who books the flight ticket, you do not want users to exit the flow in the middle of this response. Disable the barge-in option here by clicking Customize, which overrides the flow level barge-in setting. Leave the default grayed out Enable barge-in option.
...
# Scroll to the Event handlers section of the Parameter and click the Add event handler link.

# In the Event dropdown menu, select No-input 1. In the Fulfillment section, add the text "Sorry, I didn't get that. Please enter or say your ticket confirmation number."

# Event handler page with Event handler and Fulfillment options highlighted 

# Click Save to save this Event.

# Repeat these steps with the following to set up two more events.

# Event	Fulfillment	Transition
# No-input 1	Sorry, I didn't get that. Please enter or say your ticket confirmation number.	
...
# Built-in events and their levels: https://cloud.google.com/dialogflow/cx/docs/concept/handler#event-built-in
# To learn more about SSML: https://cloud.google.com/text-to-speech/docs/ssml
# See more about the Dialogflow CX Phone Gateway here: https://cloud.google.com/dialogflow/cx/docs/concept/integration/phone-gateway
# Read more about Dialogflow CX in the Dialogflow CX Documentation
# The videos on this page cover the fundamental concepts of Dialogflow CX: Video playlist
# Manual Last Updated: November 19, 2022

# Lab Last Tested: November 19, 2022
